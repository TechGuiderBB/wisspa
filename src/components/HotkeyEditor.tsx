...
type Props = {
  ...
  /**
   * Called with the changed hotkey(s) BEFORE the backend re-registration is
   * attempted. The caller owns persistence (saveSettings); this component owns
   * re-registering the live global shortcuts via updateHotkey.
   */
  onPatch: (p: Partial<Settings["hotkeys"]>) => void;
};
...
export const HOTKEY_DEFAULTS: Settings["hotkeys"] = {
...
/** Render a stored combo for display (e.g. "CmdOrCtrl+Shift+Space" → "Cmd+Shift+Space"). */
export function formatCombo(combo: string): string {
  return combo.replace(/CmdOrCtrl/g, "Cmd").replace(/Escape/g, "Esc");
}

/**
 * Shared hotkey capture editor, used by both Settings → Hotkeys and the
 * onboarding flow. Only one row captures at a time; global shortcuts are
 * paused while capturing so the keys reach the webview.
 */
export function HotkeyEditor({ hotkeys, onPatch }: Props) {
  ...
  async function commit(action: HotkeyAction, combo: string) {
    onPatch({ [action]: combo } as Partial<Settings["hotkeys"]>);
    setActiveAction(null);
    setPreviewParts([]);
    try {
  ...
  async function resetDefaults() {
    // Single patch call so only one persist fires — avoids stale-closure race
    // where 4 separate patch()+persist() calls each spread from the same
    // frozen prop and only the last write survives to disk.
    onPatch(HOTKEY_DEFAULTS);
    ...
    for (const a of HOTKEY_ACTIONS) {
      try {
        await updateHotkey(a, HOTKEY_DEFAULTS[a]);
      } catch (err) {
